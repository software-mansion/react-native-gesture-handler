...
Along the new API, version 2.0 brings one of the most requested features: [pointer events and custom gestures](./custom-gestures/custom-gestures). Thanks to great work done by the Reanimated team, we were able to provide synchronous communication between gestures and their native implementation using worklets. This allows to manage gesture state from the JS without risking race-conditions.
