...
When provided as a function, it accepts a `progress` SharedValue parameter that indicates the animation progress, similar to the [`renderNavigationView`](#rendernavigationview) function.
