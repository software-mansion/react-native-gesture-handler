...
called when pointer is down for over `delayLongPress` milliseconds (500ms by default).

after `onLongPress` has been called, `onPress` will no longer be called after lifting the pointer.
