...
called immidiately after pointer has been down for at least `delayLongPress` milliseconds (500ms by default).

after `onLongPress` has been called, `onPressOut` will be called as soon as the pointer is lifted and `onPress` will no longer be called at all.
...
additional distance outside of this view in which a press is detected and `onPressIn` is triggered.

### `pressRetentionOffset`

additional distance outside of this view (or `hitSlop` if present) in which a touch is considered a
