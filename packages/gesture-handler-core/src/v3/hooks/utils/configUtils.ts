...
>(
  ...
  config.shouldUseReanimatedDetector =
    ...
    runtime.port.reanimated !== undefined &&
...
>(
  ...
  for (const [key, value] of Object.entries(config)) {
    ...
    if (allowedNativeProps.has(key) || handlerPropsWhiteList.has(key)) {
      (filteredConfig as Record<string, unknown>)[key] = isSharedValue(value)
        ? value.value
        : value;
...
>(
  ...
  return useMemo(() => {
    ...
    resolveInternalConfigProps(runtime, transformedConfig);

    return transformedConfig;
  }, [runtime, config, propsMapping, propsTransformer]);
}
