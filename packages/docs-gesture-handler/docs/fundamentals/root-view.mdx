...
This flag controls whether Gesture Handler cancels React Native JS responders when one of Gesture Handler recognizers activates.

- `true` (default): keeps the current behavior where RN touch handlers are cancelled after Gesture Handler activates.
- `false`: disables this cancellation, so Gesture Handler callbacks and RN responder callbacks can run at the same time.

This option is currently available on iOS and Android.
