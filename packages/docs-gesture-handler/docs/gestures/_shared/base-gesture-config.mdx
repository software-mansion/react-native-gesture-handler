...
**IMPORTANT:** Note that this method only marks the relation between gestures, without [composing them](/docs/category/gesture-composition--interactions). [`GestureDetector`](/docs/fundamentals/gesture-detectors#gesture-detector) will not recognize other gestures and it needs to be added to another detector in order to be recognized.
