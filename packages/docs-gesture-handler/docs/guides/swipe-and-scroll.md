...
While we recommend using our own [`ReanimatedSwipeable`](/docs/components/reanimated_swipeable) component, creating your own version of swipeable gives you more control over its behavior. A common issue here is that after creating your own swipeable component, scroll does not work. In that case, try adding [`touchAction`](/docs/core-components/gesture-detectors#touchaction-web-only) set to `"pan-y"`, like this:
