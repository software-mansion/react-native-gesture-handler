...
A single affine matrix fixes both. It bundles move, scale and rotation into one value that you compose by multiplying matrices together in whatever order you need, and the result is itself a matrix — so each gesture simply builds on top of the previous one. You don't need to follow the underlying math to use it; the full example has the handful of small helper functions that implement it.

So keep the accumulated transform in a single matrix, and let the in-progress gesture stay as plain `scale`, `rotation` and `translation` — that's what the callbacks hand you as the fingers move. Each frame you combine the two to draw the view, and when the gesture ends you fold its values into the matrix and reset them, so the next gesture picks up where it left off.
