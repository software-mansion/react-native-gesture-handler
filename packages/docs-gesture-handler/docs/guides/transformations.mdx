...
So keep the result so far in a single matrix, and let the in-progress gesture stay as plain `scale`, `rotation` and `translation` — that's what the callbacks hand you as the fingers move. Each frame you combine the two to draw the view, and when the gesture ends you fold its values into the matrix and reset them, so the next gesture picks up where it left off.
