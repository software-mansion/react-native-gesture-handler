...
As described in ["About Gesture Handlers"](/docs/2.x/gesture-handlers/about-handlers), gesture handlers can be treated as ["state machines"](https://en.wikipedia.org/wiki/Finite-state_machine).
...
We can monitor a handler's state changes by using the [`onHandlerStateChange`](/docs/2.x/gesture-handlers/common-gh#onhandlerstatechange) callback and the destructured `nativeEvent` argument passed to it.
This can be done by comparing the `nativeEvent`'s [`state`](/docs/2.x/gesture-handlers/common-gh#state) attribute to one of the constants exported under the `State` object (see example below).
...
Learn about [discrete and continuous handlers here](/docs/2.x/gesture-handlers/about-handlers#discrete-vs-continuous) to understand how long a handler can be kept in the `ACTIVE` state.
