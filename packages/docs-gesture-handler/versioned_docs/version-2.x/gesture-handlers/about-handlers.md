...
Each handler works as an isolated state machine. It takes a touch stream as an input and based on it, it can flip between [states](/docs/2.x/under-the-hood/state).
...
Keep in mind that `onGestureEvent` is only generated by continuous gesture handlers and shouldn't be used in the `TapGestureHandler` and other discrete handlers.
