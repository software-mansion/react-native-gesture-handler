A plain gesture that has no specific activation criteria nor event data set. Its [state](/docs/under-the-hood/state) has to be controlled manually using a [state manager](/docs/fundamentals/state-manager). It will not fail when all the pointers are lifted from the screen.

> **Tip**
>
> If you need to modify the activation criteria of gestures other than `Manual`, check their configuration for relevant properties or use [`manualActivation`](/docs/fundamentals/state-manager#manualactivation) to manage their state directly.
...
We have our components set up and we can finally get to making the gesture! We will start with [`onTouchesDown`](/docs/fundamentals/callbacks-events#ontouchesdown) where we need to set position of the pointers and make them visible. We can get this information from the touches property of the event. In this case we will also check how many pointers are on the screen and activate the gesture if there are at least two.

Next, we will handle pointer movement. In [`onTouchesMove`](/docs/fundamentals/callbacks-events#ontouchesmove) we will simply update the position of moved pointers.

We also need to handle lifting fingers from the screen, which corresponds to [`onTouchesUp`](/docs/fundamentals/callbacks-events#ontouchesup). Here we will just hide the pointers that were lifted and end the gesture if there are no more pointers on the screen.
Note that we are not handling [`onTouchesCancel`](/docs/fundamentals/callbacks-events#ontouchescancel) as in this very basic case we don't expect it to happen, however you should clear data about cancelled pointers (most of the time all active ones) when it is called.
