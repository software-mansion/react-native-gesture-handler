...
read_plan() {
  ...
  GESTURE_HANDLER_VERSION=$(plan '.gestureHandlerVersion')
...
install_library_packages() {
  local packages=("react-native-gesture-handler@$GESTURE_HANDLER_VERSION")
  while IFS= read -r dependency; do
...
write_babel_config() {
  ...
  local plugins=""
  if has_extra_dependency react-native-worklets; then
    plugins="'react-native-worklets/plugin'"
  elif has_extra_dependency react-native-reanimated; then
...
has_extra_dependency() {
  grep -q -- "^$1@" <<< "$EXTRA_DEPENDENCIES"
}
