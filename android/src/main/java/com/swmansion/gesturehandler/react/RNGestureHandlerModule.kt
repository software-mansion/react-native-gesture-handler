...
class RNGestureHandlerModule(reactContext: ReactApplicationContext?) : ReactContextBaseJavaModule(reactContext) {
  ...
  @ReactMethod
  fun attachGestureHandlerWithReceiver(handlerTag: Int, viewTag: Int, receiverTag: Int) {
    tryInitializeHandlerForReactRootView(viewTag)
    if (!registry.attachHandlerToViewWithReceiver(handlerTag, viewTag, receiverTag)) {
      throw JSApplicationIllegalArgumentException("Handler with tag $handlerTag does not exists")
    }
  }
  ...
  private fun <T : GestureHandler<T>> onTouchEvent(handler: T, motionEvent: MotionEvent) {
    ...
    if (handler.state == GestureHandler.STATE_ACTIVE) {
      ...
      if (handler.receiverTag == -1) {
        ...
        args.putInt("handlerTag", handler.tag)
        args.putInt("state", handler.state)

        reactApplicationContext
          .getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter::class.java)
          .emit("onGestureHandlerEvent", args)
      } else {
        reactApplicationContext
  ...
  private fun <T : GestureHandler<T>> onStateChange(handler: T, newState: Int, oldState: Int) {
    ...
    if (handler.receiverTag == -1) {
      ...
      reactApplicationContext
        .getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter::class.java)
        .emit("onGestureHandlerStateChange", args)
    } else {
      reactApplicationContext
        .getNativeModule(UIManagerModule::class.java)!!
        .eventDispatcher.let {
